...
class StableDiffusionHandler:
    ...
    def get_misc(self):
        ...
        create_ImageNetFolder(root_dir=f'{self.args.imagenet_path}/train', out_dir=f"{self.args.syn_path}/train5")#这里改合成数据的路径
    ...
    def generate_ImageNet1k(self):
        ...
        for i, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            bs = len(image_paths)
            out_paths = [os.path.join(f"{self.args.syn_path}/train5",f'{image_names[idx]}.jpg') for idx in range(bs)]#其实是这里改啦
